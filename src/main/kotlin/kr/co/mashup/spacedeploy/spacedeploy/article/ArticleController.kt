...
class ArticleController(val articleService: ArticleService) {
    ...
    @GetMapping()
    fun getArticle(@RequestParam year: Int, @RequestParam month: Int, @RequestParam day: Int, request: HttpServletRequest): ResponseEntity<ResArticleDto> {
    ...
    @PutMapping()
    fun editArticle(@PathVariable dailyLogId: Long, request: HttpServletRequest) {
        val token = request.getHeader("Authorization")
        ResponseEntity.status(HttpStatus.OK)
    }
}
